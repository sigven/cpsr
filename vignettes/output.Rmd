...
We provide an Excel workbook with **four** sheets that cover the main findings of the predisposition analysis
...
| 5. `GENOTYPE` | Variant genotype (heterozygous/homozygous) |
| 6. `CPSR_CLASSIFICATION_SOURCE` | ClinVar or CPSR_ACMG (the latter meaning variant not recorded in ClinVar, classified by CPSR) |
...
| 20. `PROTEIN_CHANGE` | Protein change - one letter abbreviation (HGVSp) |
| 21. `PFAM_DOMAIN_NAME` | Protein domain name (Pfam) |
| 22. `HGVSp` | The HGVS protein sequence name |
| 23. `HGVSc` | The HGVS coding sequence name |
| 24. `CDS_CHANGE` | Coding, transcript-specific sequence annotation |
| 25. `LAST_EXON` | Last exon in gene |
| 26. `EXON`| Exon of variant/total number of exons in transcript (from VEP) |
| 27. `EXON_AFFECTED` | Transcript exon of variant (from VEP) | 
...
| 30. `VEP_ALL_CSQ` | All VEP transcript block consequences |
| 31. `CANCER_PHENOTYPE` | For variants with a ClinVar classification, indication of cancer-associated disease/phenotype (1) or not (0) |
| 32. `MUTATION_HOTSPOT` | Cancer mutation hotspot (cancerhotspots.org) |
| 33. `RMSK_HIT` | RepeatMasker hit |
| 34. `EFFECT_PREDICTIONS` | Functional effect predictions from multiple algorithms (dbNSFP) |
| 35. `LOSS_OF_FUNCTION` | Loss-of-function variant, as predicted from VEP's LofTee plugin |
| 36. `NULL_VARIANT` | Frameshift or stop-gain variant |
| 37. `DBMTS` | variant with potential effect on microRNA target sites (dbMTS). Format: `<ensembl_transcript_id>|<microrna_identifier>|<target_prediction_algorithms>|<gain_loss_consensus>`. _Target prediction algorithms_ indicate support by different algorithms (separated by '&'), `TS` = TargetScan, `M` = miRanda, `R` = RNAhybrid. *Gain_loss_consensus* indicate whether the variant was predicted to disrupt a binding site (`L` = Loss), or create a new target site (`G` = gain) by the different algorithms |
| 38. `REGULATORY_ANNOTATION` | Overlap of variant with regulatory elements (VEP) | 
| 39. `TF_BINDING_SITE_VARIANT` | Indicates whether a variant overlaps a critical/non-critical position of a transcription factor binding site (TFBS) - as provided by VEP's--regulatory option ('Overlap: non-critical motif position' or 'Overlap: critical motif position') |
| 40. `TF_BINDING_SITE_VARIANT_INFO` | Comma-separated list of transcription factor binding sites affected by variant. Format per factor: `<TRANSCRIPTION_FACTOR>|<MOTIF_NAME>|<MOTIF_POS>|<MOTIF_SCORE_CHANGE>|<HIGH_INF_POS>`. *HIGH_INF_POS* indicates whether the variant overlapped a critical motif position (`Y`), or non-critical motif position (`N`) |
| 41. `GERP_SCORE` | Genomic conservation score (GERP) |
| 42. `DBSNP_RSID` | dbSNP identifier (rsid) |
| 43. `CLINVAR_CLASSIFICATION` | clinical significance of ClinVar-recorded variant |
| 44. `CLINVAR_MSID` | measureset identifier of ClinVar variant |
| 45. `CLINVAR_VARIANT_ORIGIN` | variant origin (somatic/germline) of ClinVar variant |
| 46. `CLINVAR_CONFLICTED` | indicator of conflicting interpretations |
| 47. `CLINVAR_PHENOTYPE` | associated phenotype(s) for ClinVar variant |
| 48. `CLINVAR_REVIEW_STATUS_STARS` | Review confidence - number of gold stars |
| 49. `N_INSILICO_CALLED` | Number of algorithms with effect prediction (damaging/tolerated) from dbNSFP |
| 50. `N_INSILICO_DAMAGING` | Number of algorithms with damaging prediction from dbNSFP |
| 51. `N_INSILICO_TOLERATED` | Number of algorithms with tolerated prediction from dbNSFP |
| 52. `N_INSILICO_SPLICING_NEUTRAL` | Number of algorithms with splicing neutral prediction from dbscSNV |
| 53. `N_INSILICO_SPLICING_AFFECTED` | Number of algorithms with splicing affected prediction from dbscSNV |
| 54. `gnomADe_AF` | Global MAF in gnomAD (exome samples) |
| 55. `FINAL_CLASSIFICATION` | Final variant classification based on the combination of `CLINVAR_CLASSIFICTION` (for ClinVar-classified variants), and `CPSR_CLASSIFICATION` (for novel variants) |
| 56. `CPSR_CLASSIFICATION` | Variant clinical significance by CPSR's classification algorithm (P/LP/VUS/LB/B) |
| 57. `CPSR_PATHOGENICITY_SCORE` | Aggregated pathogenicity score by CPSR's algorithm |
| 58. `CPSR_CLASSIFICATION_CODE` | Combination of CPSR classification codes assigned to the variant (ACMG) |
| 59. `CPSR_CLASSIFICATION_DOC` | Descriptions of CPSR classification codes assigned to the variant (ACMG) |
| 60. `<CUSTOM_POPULATION_GNOMAD`> | Population specific MAF in gnomAD control (non-cancer, population configured by user) |
