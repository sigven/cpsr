...
| 40. `TF_BINDING_SITE_VARIANT` | Indicates whether a variant overlaps a critical/non-critical position of a transcription factor binding site (TFBS) - as provided by VEP's--regulatory option ('Overlap: non-critical motif position' or 'Overlap: critical motif position') |
| 41. `TF_BINDING_SITE_VARIANT_INFO` | Comma-separated list of transcription factor binding sites affected by variant. Format per factor: `<TRANSCRIPTION_FACTOR>|<MOTIF_NAME>|<MOTIF_POS>|<MOTIF_SCORE_CHANGE>|<HIGH_INF_POS>`. *HIGH_INF_POS* indicates whether the variant overlapped a critical motif position (`Y`), or non-critical motif position (`N`) |
| 42. `GERP_SCORE` | Genomic conservation score (GERP) |
| 43. `DBSNP_RSID` | dbSNP identifier (rsid) |
| 44. `CLINVAR_CLASSIFICATION` | clinical significance of ClinVar-recorded variant |
| 45. `CLINVAR_MSID` | measureset identifier of ClinVar variant |
| 46. `CLINVAR_VARIANT_ORIGIN` | variant origin (somatic/germline) of ClinVar variant |
| 47. `CLINVAR_CONFLICTED` | indicator of conflicting interpretations |
| 48. `CLINVAR_PHENOTYPE` | associated phenotype(s) for ClinVar variant |
| 49. `CLINVAR_REVIEW_STATUS_STARS` | Review confidence - number of gold stars |
| 50. `N_INSILICO_CALLED` | Number of algorithms with effect prediction (damaging/tolerated) from dbNSFP |
| 51. `N_INSILICO_DAMAGING` | Number of algorithms with damaging prediction from dbNSFP |
| 52. `N_INSILICO_TOLERATED` | Number of algorithms with tolerated prediction from dbNSFP |
...
| 60. `CPSR_CLASSIFICATION_DOC` | Descriptions of CPSR classification codes assigned to the variant (ACMG) |
| 61. `<CUSTOM_POPULATION_GNOMAD`> | Population specific MAF in gnomAD control (non-cancer, population configured by user) |
