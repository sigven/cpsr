...
| 9.  `ACMG_BP4` (**-1**) | Multiple lines of computational evidence (>=8) algorithmssupport a benign effect on the gene or gene product (conservation, evolutionary, splicing impact, etc. - from dbNSFP |
...
| 15. `ACMG_PVS1_1` (**5**) | Null variant (frameshift/nonsense) - classified as loss-of-function - within pathogenic range - LoF established for gene |
| 16. `ACMG_PVS1_2` (**2.5**) | Null variant (frameshift/nonsense) - not classified as loss-of-function - within pathogenic range - LoF established for gene |
| 17. `ACMG_PVS1_3` (**2**) | Null variant (frameshift/nonsense) - classified as loss-of-function - within pathogenic range - LoF not established for gene |
| 18. `ACMG_PVS1_4` (**0**) | Null variant (frameshift/nonsense) - not classified as loss-of-function -- within pathogenic range - LoF not established for gene |
...
| 21. `ACMG_PVS1_7` (**4**) | Donor/acceptor variant - classified as loss-of-function - within pathogenic range - not last intron - LoF established for gene |
...
| 26. `ACMG_PP2` (**0.5**) | Missense variant in a gene that has a relatively low rate of benign missense variation (<10%) and where missense variants are a common mechanism of disease (>50% P/LP (ClinVar)) |
...
| 31. `ACMG_PP3` (**0.5**) | Multiple lines of computational evidence (>=8) algorithms support a deleterious effect on the gene or gene product (conservation, evolutionary, splicing impact, etc. - from dbNSFP |

<br>
Currently (as of 2024-09), based on a calibration against ClinVar-classified variants (minimum two review status stars) in n = 105 core cancer predisposition genes, the clinical significance (**CPSR_CLASSIFICATION**) is determined based on the following ranges of pathogenicity scores:
