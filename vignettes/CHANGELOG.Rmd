...
- Major data updates
  ...
  - Complete revision and much simplified implementation for ACMG/AMP variant classification, 
    ...
    considering e.g. 
      * gene-specific allele frequency thresholds for different criteria (BA1/BS1/PM2)
...
- Interactive HTML report tables migrated from *DT* (`DT::datatable`) to *reactable* (`reactable::reactable`):
  ...
  - Classification table retains linked crosstalk filters (gene panel, consequence, genotype, gnomAD AF) wired to a shared data object
  - Unified dark header theme (`#2c313c`) across secondary findings, pharmacogenomics, and biomarker tables; teal header (`#007a74`) for the main classification table
  - Clinical significance column rendered as colour-coded pill badges matching the established pathogenicity palette (P/LP/VUS/LB/B)
  ...
  - gnomAD allele frequency displayed in scientific notation with sort order preserved via numeric backing column
