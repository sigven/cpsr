...
version = '0.1.0'
db_version = 'PCGR_DB_VERSION = 20180815'
vep_version = '93'
global vep_assembly
...
def __main__():
   ...
   docker_image_version = 'sigven/pcgr:0.6.3'
   ...
   run_cpsr(host_directories, docker_image_version, config_options, args.sample_id, args.genome_assembly, version, args.basic)
...
def run_cpsr(host_directories, docker_image_version, config_options, sample_id, genome_assembly, version, basic):
   """
   ...
   vcf_validate_command = str(docker_command_run1) + "cpsr_validate_input.py /data " + str(input_vcf_docker) + " " + str(input_conf_docker) + " " + str(genome_assembly) + "\""
   ...
   if not input_vcf_docker == 'None':
      ...
      input_vcf_cpsr_ready = '/workdir/output/' + re.sub(r'(\.vcf$|\.vcf\.gz$)','.cpsr_ready.vcf.gz',host_directories['input_vcf_basename_host'])
      input_vcf_cpsr_ready_uncompressed = '/workdir/output/' + re.sub(r'(\.vcf$|\.vcf\.gz$)','.cpsr_ready.vcf',host_directories['input_vcf_basename_host'])
      vep_vcf = re.sub(r'(\.vcf$|\.vcf\.gz$)','.pcgr_vep.vcf',input_vcf_cpsr_ready)
      vep_vcfanno_vcf = re.sub(r'(\.vcf$|\.vcf\.gz$)','.pcgr_vep.vcfanno.vcf',input_vcf_cpsr_ready)
      ...
      fasta_assembly = os.path.join("/usr/local/share/vep/data", "homo_sapiens", str(vep_version) + "_" + str(vep_assembly), "Homo_sapiens." + str(vep_assembly) + ".dna.primary_assembly.fa.gz")
      ...
      vep_main_command = str(docker_command_run1) + "vep --input_file " + str(input_vcf_cpsr_ready) + " --output_file " + str(vep_tmp_vcf) + " " + str(vep_options) + " --fasta " + str(fasta_assembly) + "\""
      ...
      pcgr_vcfanno_command = str(docker_command_run2) + "pcgr_vcfanno.py --num_processes "  + str(config_options['other']['n_vcfanno_proc']) + " --dbnsfp --clinvar --civic --uniprot --pcgr_onco_xref " + str(vep_vcf) + ".gz " + str(vep_vcfanno_vcf) + " /data/data/" + str(genome_assembly) + "\""
      ...
      pcgr_summarise_command = str(docker_command_run2) + "pcgr_summarise.py " + str(vep_vcfanno_vcf) + ".gz /data/data/" + str(genome_assembly) + " --cpsr\""
      ...
      clean_command = str(docker_command_run2) + 'rm -f ' + str(vep_vcf) + '* ' + str(vep_vcfanno_annotated_vcf) + ' ' + str(vep_vcfanno_annotated_pass_vcf) + '* ' + str(vep_vcfanno_vcf) + '* ' +  str(input_vcf_cpsr_ready_uncompressed) + "* "  + "\""
   ...
   if not basic: 
      ...
      pcgr_report_command = str(docker_command_run1) + "/cpsr.R /workdir/output " + str(output_pass_tsv) + ".gz " +  str(sample_id)  + " " + str(input_conf_docker) + " " + str(version) + " " + str(genome_assembly) + " /data " + "\""
