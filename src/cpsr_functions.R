...
generate_cpsr_report <- function(project_directory, query_vcf2tsv,
  ...
  invisible(assertthat::assert_that(!is.null(cpsr_config), msg = "Object 'cpsr_config' cannot be NULL"))
  cps_report <- pcgrr::init_report(cpsr_config, sample_name, class = NULL,
  ...
  class_1_5_display <- c("SYMBOL", "SOURCE", "CLINVAR_PHENOTYPE", "CONSEQUENCE", "PROTEIN_CHANGE",
                         "GENOTYPE", "GENE_NAME", "PROTEIN_DOMAIN",
                         ...
                         "CLINVAR_VARIANT_ORIGIN", "CPSR_CLASSIFICATION", "CPSR_CLASSIFICATION_SCORE",
  ...
  ## define tags/variables to display in data tables (incidental findings)
  incidental_findings_display <- c("SYMBOL", "CONSEQUENCE","CLINVAR_CLASSIFICATION", "CLINVAR_PHENOTYPE",
  ...
  predispose_gwas_display <- c("SYMBOL", "CONSEQUENCE", "GWAS_CITATION", "PROTEIN_CHANGE", "GENOTYPE",
                               "LOSS_OF_FUNCTION", "PROTEIN_CHANGE", "GENE_NAME", "GWAS_PHENOTYPE",
                               "PROTEIN_DOMAIN", "HGVSp", "HGVSc", "NCBI_REFSEQ", "CDS_CHANGE", "CODING_STATUS",
                               "REFSEQ_MRNA", "PROTEIN_FEATURE", "PREDICTED_EFFECT", "DBSNP", "GLOBAL_AF_GNOMAD",
                               "GENOMIC_CHANGE", "GENOME_VERSION")
  ...
  if (query_vcf2tsv != "None.gz") {
    if (!file.exists(query_vcf2tsv) | file.size(query_vcf2tsv) == 0) {
      ...
      if (!is.null(cpsr_config) & query_vcf2tsv != "None.gz") {
        ...
        ## read calls
        calls <- pcgrr::get_calls(query_vcf2tsv, pcgr_data, sample_name,
                                  cpsr_config,
                                  oncotree = cps_report[["metadata"]][["phenotype_ontology"]][["oncotree_query"]],
                                  cpsr = TRUE)
        if (nrow(calls) == 0) {
        ...
        if(cpsr_config[['ignore_noncoding']] == T){
          n_noncoding_vars <- calls %>% dplyr::filter(CODING_STATUS == "noncoding") %>% NROW()
          rlogging::message("Excluding n = ",n_noncoding_vars," variants for classification/reporting (option --ignore_noncoding)")
          calls <- calls %>% dplyr::filter(CODING_STATUS == "coding")
          if (NROW(calls) == 0) {
            rlogging::warning("There are zero remaining protein-coding calls in input file - no report will be produced")
            return(NULL)
          }
        ...
        cpg_calls <- dplyr::inner_join(calls, cps_report[["metadata"]][["gene_panel"]][["genes"]],
                                       by = c("SYMBOL" = "symbol"))
        cpg_call_stats <- pcgrr::variant_stats_report(cpg_calls, name = "variant_statistic_cpg")

        rlogging::message(paste0("Number of coding variants in cancer predisposition genes: ",
        ...
        gene_hits <- paste(unique(cps_report[["content"]][["snv_indel"]][["variant_set"]][["tsv"]]$SYMBOL),
                           collapse = ", ")
...
get_max_rows_pr_datatable <- function(cps_report){
  ...
  max_row_nr <- 0
  if(!is.null(cps_report[['content']][['snv_indel']][['variant_display']])){
    for(c in c('class1','class2','class3','class4','class5','sf','gwas')){
      if(NROW(cps_report[['content']][['snv_indel']][['variant_display']][[c]]) == 0){
        next
      }
      t1 <- cps_report[['content']][['snv_indel']][['variant_display']][[c]]
      if(c == 'sf' | c == 'gwas'){
        ...
        t1 <- cps_report[['content']][['snv_indel']][['variant_display']][[c]]
        num_rows_clinvar <- t1 %>% dplyr::filter(SOURCE == "ClinVar") %>% nrow()
        num_rows_other <- t1 %>% dplyr::filter(SOURCE == "Other") %>% nrow()
...
get_insilico_prediction_statistics <- function(cpg_calls) {
  ...
  insilico_pathogenicity_pred_algos <- c("SIFT_DBNSFP", "PROVEAN_DBNSFP",
...
assign_pathogenicity_evidence <- function(cpg_calls, cpsr_config, pcgr_data) {
  ...
  pathogenic_range_af <- 0.0005
  min_an <- 12000

  acmg_ev_codes <- c("ACMG_BA1_AD",   ## Very high MAF (> 0.5% in gnomAD non-cancer pop subset) - min AN = 12,000, - Dominant mechanism of disease
                     "ACMG_BS1_1_AD", ## High MAF (> 0.1% in gnomAD non-cancer pop subset) - min AN = 12,000 - Dominant mechanism of disease
                     "ACMG_BS1_2_AD", ## Somewhat high MAF (> 0.005% in gnomAD non-cancer pop subset) - min AN = 12,000 - Dominant mechanism of disease
                     "ACMG_BA1_AR",   ## Very high MAF (> 1% in gnomAD non-cancer pop subset) - min AN = 12,000 - Recessive mechanism of disease
                     "ACMG_BS1_1_AR", ## High MAF (> 0.3% in gnomAD non-cancer pop subset) - min AN = 12,000 - Recessive mechanism of disease
                     "ACMG_BS1_2_AR", ## Somewhat high MAF (> 0.005% in gnomAD non-cancer pop subset) - min AN = 12,000 - Recessive mechanism of disease
                     ...
                     "ACMG_PM2_1",    ## Allele count within pathogenic range (MAF < 0.005% in the population-specific non-cancer gnomAD subset, min AN = 12,000)
                     ...
                     "ACMG_PVS1_10",  ## donor variant at located at the +3, +4 or +5 position of the intron -  within the pathogenic range (i.e. MAF < 0.005% in gnOMAD))
  ...
  path_columns <- c(acmg_ev_codes, "N_INSILICO_CALLED",
                    ...
                    "clinvar_pathogenic", "clinvar_benign", "clinvar_benign_codon", "hotspot_symbol",
  ...
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(cpsr_gene_moi = dplyr::if_else(stringr::str_detect(CANCER_PREDISPOSITION_MOI, "AD|AD/AR"),
                                                 "AD", as.character(NA), as.character(NA))) %>%
    dplyr::mutate(cpsr_gene_moi = dplyr::if_else(!stringr::str_detect(CANCER_PREDISPOSITION_MOI, "AD|Mosaic"),
                                                 "AR", cpsr_gene_moi, cpsr_gene_moi))

  predisposition_gene_info <- dplyr::select(pcgr_data[["predisposition"]][["genes"]], symbol,
                                            mechanism_of_disease, path_truncation_rate, benign_missense_rate) %>%
  ...
  #
  #
  # ACMG_PP3 - Multiple lines (>=5) of insilico evidence support a deleterious effect on the gene or gene product
  ...
  # 1) Damaging: Among all possible protein variant effect predictions, at least six algorithms must have made a call,
  #              with at least 5 predicted as damaging, and at most two predicted as tolerated (PP3)
  ...
  # 2) Tolerated: Among all possible protein variant effect predictions, at least six algorithms must have made a call,
  #    with at least 5 predicted as tolerated, and at most one predicted as damaging (BP4)
  ...
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_PP3 = dplyr::if_else(N_INSILICO_CALLED >= dbnsfp_min_called &
    ...
    dplyr::mutate(ACMG_BP4 = dplyr::if_else(N_INSILICO_CALLED >= dbnsfp_min_called &
    ...
    dplyr::mutate(ACMG_PP3 = dplyr::case_when(N_INSILICO_SPLICING_AFFECTED == 2 ~ TRUE, TRUE ~ as.logical(ACMG_PP3)))
  ...
  # 'ACMG_BA1_AD'   - Very high MAF (> 0.5% in gnomAD non-cancer pop subset) - min AN = 12,000 - Dominant mechanism of disease
  # 'ACMG_BS1_1_AD' - High MAF (> 0.1% in gnomAD non-cancer pop subset) - min AN = 12,000 - Dominant mechanism of disease
  # 'ACMG_BS1_2_AD' - Somewhat high MAF (> 0.005% in gnomAD non-cancer pop subset) - Dominant mechanism of disease
  # 'ACMG_BA1_AR'   - Very high MAF (> 1% in gnomAD non-cancer pop subset) - min AN = 12,000 - Recessive mechanism of disease
  # 'ACMG_BS1_1_AR' - High MAF (> 0.3% in gnomAD non-cancer pop subset) - min AN = 12,000 - Recessive mechanism of disease
  # 'ACMG_BS1_2_AR' - Somewhat high MAF (> 0.005% in gnomAD non-cancer pop subset) - Recessive mechanism of disease
  # 'ACMG_PM2_1'    - Allele count within pathogenic range (MAF <= 0.005% in the population-specific non-cancer gnomAD subset, min AN = 12,000)
  # 'ACMG_PM2_2'    - Alternate allele absent in the population-specific non-cancer gnomAD subset
  ...
  # 'ACMG_PVS1_5' - start lost - within pathogenic range - Lof established for gene
  # 'ACMG_PVS1_6' - start lost - within pathogenic range - LoF not established for gene
  # 'ACMG_PVS1_7' - splice acceptor/donor variant - predicted as LoF by LOFTEE - not last intron - within pathogenic range - Lof established for gene
  # 'ACMG_PVS1_8' - splice acceptor/donor variant - predicted as LoF by LOFTEE - last intron - within pathogenic range - Lof established for gene
  # 'ACMG_PVS1_9' - splice acceptor/donor variant - predicted as LoF by LOFTEE - not last intron - within pathogenic range - Lof established for gene
  # 'ACMG_PVS1_10' - splice variant involving a donor at +3A/G, +4A or +5G - predicted as damaging by insilico predictions - within pathogenic range
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_PVS1_1 = dplyr::if_else(NULL_VARIANT == T & LOSS_OF_FUNCTION == T &
                                                 MOD == "LoF" &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_3 = dplyr::if_else(NULL_VARIANT == T & LOSS_OF_FUNCTION == T &
                                                 (is.na(MOD) | MOD != "LoF") &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_2 = dplyr::if_else(NULL_VARIANT == T & LOSS_OF_FUNCTION == F &
                                                 MOD == "LoF" &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_4 = dplyr::if_else(NULL_VARIANT == T & LOSS_OF_FUNCTION == F &
                                                 (is.na(MOD) | MOD != "LoF") &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_5 = dplyr::if_else(CONSEQUENCE == "start_lost" & MOD == "LoF" &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_6 = dplyr::if_else(CONSEQUENCE == "start_lost" & (is.na(MOD) | MOD != "LoF") &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_7 = dplyr::if_else(LOSS_OF_FUNCTION == T &
                                                 stringr::str_detect(CONSEQUENCE, "_donor|_acceptor") &
                                                 LAST_INTRON == F & MOD == "LoF" &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_8 = dplyr::if_else(LOSS_OF_FUNCTION == T &
                                                 stringr::str_detect(CONSEQUENCE, "_donor|_acceptor") &
                                                 LAST_INTRON == T & MOD == "LoF" &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_9 = dplyr::if_else(LOSS_OF_FUNCTION == T &
                                                 stringr::str_detect(CONSEQUENCE, "_donor|_acceptor") &
                                                 LAST_INTRON == F & (is.na(MOD) | MOD != "LoF") &
                                                 (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                               TRUE, FALSE, FALSE)) %>%
    dplyr::mutate(ACMG_PVS1_10 = dplyr::if_else(SPLICE_DONOR_RELEVANT == T & ACMG_PP3 == TRUE &
                                                  (ACMG_PM2_1 == TRUE | ACMG_PM2_2 == TRUE),
                                                TRUE, FALSE,  FALSE))
  ...
  # Assign logical ACMG evidence indicators
  # # TODO - BA1 -  exceptions for high population germline frequency (gnomAD) - HFE/SERPINA1

  ## Assign logical ACMG evidence indicator
  # PM4 - Protein length changes (in non-repetitive regions) due to inframe indels
  #       or nonstop variant of genes that harbor variants with a dominant mode of inheritance - PM4
  # PPC1 - Protein length changes (in non-repetitive regions) due to inframe indels or nonstop variant of genes that
  #        harbor variants with a recessive mode of inheritance (and unknown MOI) - PPC1
  if ("RMSK_HIT" %in% colnames(cpg_calls)) {
    cpg_calls <- cpg_calls %>%
      dplyr::mutate(ACMG_PM4 =
                      dplyr::if_else(stringr::str_detect(CONSEQUENCE, "stop_lost|inframe_deletion|inframe_insertion") &
                                       is.na(RMSK_HIT) & cpsr_gene_moi == "AD",
                                     TRUE, FALSE, FALSE)) %>%
      dplyr::mutate(ACMG_PPC1 =
                      dplyr::if_else(stringr::str_detect(CONSEQUENCE, "stop_lost|inframe_deletion|inframe_insertion") &
  ...
  ## Assign logical ACMG evidence indicator
  # ACMG_PP2 - Missense variant in a gene that has a relatively low rate of benign
  #            missense variation and where missense variants are a common mechanism of disease
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_PP2 = dplyr::if_else((is.na(BENIGN_MISSENSE_RATE) | BENIGN_MISSENSE_RATE <= 0.3) &
  ...
  # ACMG_BP1 - Missense variant in a gene for which primarily truncating variants (> 90%, as given in Maxwell et al.)
  #            are known to cause disease
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_BP1 = dplyr::if_else(PATH_TRUNCATION_RATE > 0.90 &
                                              stringr::str_detect(CONSEQUENCE, "^missense_variant"),
                                            TRUE, FALSE, FALSE))
  ...
  # ACMG_BP7 - Silent/intronic variant outside of the splice site consensus
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_BP7 = dplyr::if_else(((INTRON_POSITION < 0 & INTRON_POSITION < -3) |
                                               (INTRON_POSITION > 0 & INTRON_POSITION > 6) |
                                               (EXON_POSITION < 0 & EXON_POSITION < -2) |
                                               (EXON_POSITION > 0 & EXON_POSITION > 1)) &
                                              stringr::str_detect(CONSEQUENCE, "^(synonymous_variant|intron_variant|splice_region_variant)"),
                                            TRUE, FALSE, FALSE))

  ## Assign a logical ACMG evidence indicator
  # ACMG_BP3 - Variants in promoter or untranslated regions
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_BP3 = dplyr::if_else(stringr::str_detect(CONSEQUENCE, "^(downstream|upstream|5_prime_UTR_variant|3_prime_UTR_variant)"),
                                            TRUE, FALSE, FALSE))
  ...
  if (nrow(cpg_calls[!is.na(cpg_calls$codon_prefix), ]) > 0) {
    cpg_calls_pathogenic_codon <-
      dplyr::left_join(dplyr::filter(dplyr::select(cpg_calls, VAR_ID, codon_prefix, SYMBOL), !is.na(codon_prefix)),
                       pcgr_data[["clinvar"]][["cpg_loci"]][["high_confidence"]][["pathogenic"]][["codon"]],
  ...
  if (nrow(cpg_calls[!is.na(cpg_calls$HGVSp_short), ]) > 0) {
    cpg_calls_pathogenic_hgvsp <-
      dplyr::left_join(dplyr::filter(dplyr::select(cpg_calls, VAR_ID, HGVSp_short, SYMBOL), !is.na(HGVSp_short)),
                       pcgr_data[["clinvar"]][["cpg_loci"]][["high_confidence"]][["pathogenic"]][["peptide_change"]],
    ...
    cpg_calls_benign_hgvsp <-
      dplyr::left_join(dplyr::filter(dplyr::select(cpg_calls, VAR_ID, HGVSp_short, SYMBOL), !is.na(HGVSp_short)),
                       pcgr_data[["clinvar"]][["cpg_loci"]][["high_confidence"]][["benign"]][["peptide_change"]],
                       by = c("HGVSp_short" = "hgvs_p", "SYMBOL" = "symbol"))
    cpg_calls <- cpg_calls %>%
      dplyr::left_join(dplyr::select(cpg_calls_benign_hgvsp, VAR_ID, clinvar_benign),
                       by = c("VAR_ID")) %>%
      dplyr::mutate(ACMG_PS1 = dplyr::if_else(clinvar_pathogenic == TRUE, TRUE, FALSE, FALSE)) %>%
      dplyr::mutate(ACMG_BSC1 = dplyr::if_else(clinvar_benign == TRUE, TRUE, FALSE, FALSE))
  }else{
  ...
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(ACMG_PM5 = dplyr::case_when(ACMG_PM5 == T & ACMG_PS1 == T ~ FALSE,
                                              TRUE ~ as.logical(ACMG_PM5))) %>%
    ## if previously found coinciding with benign variant (ACMG_BSC1), set ACMG_BMC1 to false
    dplyr::mutate(ACMG_BMC1 = dplyr::case_when(ACMG_BMC1 == T & ACMG_BSC1 == T ~ FALSE,
                                               TRUE ~ as.logical(ACMG_BMC1))) %>%
    dplyr::select(-c(clinvar_pathogenic_codon, clinvar_benign_codon, clinvar_pathogenic,
  ...
  cpg_calls <- cpg_calls %>%
    tidyr::separate(MUTATION_HOTSPOT, c("hotspot_symbol", "hotspot_codon", "hotspot_pvalue"),
                    sep = "\\|", remove = F, extra = "drop") %>%
    dplyr::mutate(hotspot_docon = dplyr::if_else(!is.na(hotspot_codon),
                                                 paste0("p.", hotspot_codon),
                                                 as.character(NA))) %>%
    dplyr::mutate(ACMG_PM1 = dplyr::if_else(!is.na(hotspot_codon) & !is.na(hotspot_symbol) &
                                              !is.na(codon_prefix) &
                                              SYMBOL == hotspot_symbol & hotspot_codon == codon_prefix,
                                            TRUE, FALSE))
...
determine_pathogenicity_classification <- function(cpg_calls) {
  ...
  while (i <= nrow(evidence_codes)) {
    ...
    if (cpsr_evidence_code %in% colnames(cpg_calls)) {
      cpg_calls <- cpg_calls %>%
        dplyr::mutate(cpsr_score_benign = cpsr_score_benign +
                        dplyr::if_else(pole == "B" & !!rlang::sym(cpsr_evidence_code) == T, score, 0)) %>%
        dplyr::mutate(cpsr_score_pathogenic = cpsr_score_pathogenic +
                        dplyr::if_else(pole == "P" & !!rlang::sym(cpsr_evidence_code) == T, score, 0)) %>%
        dplyr::mutate(CPSR_CLASSIFICATION_DOC =
                        paste0(CPSR_CLASSIFICATION_DOC, dplyr::if_else(!!rlang::sym(cpsr_evidence_code) == T,
                                                                       paste0("- ", description), ""), sep = "<br>")) %>%
        dplyr::mutate(CPSR_CLASSIFICATION_CODE =
                        paste0(CPSR_CLASSIFICATION_CODE, dplyr::if_else(!!rlang::sym(cpsr_evidence_code) == T,
  ...
  cpg_calls <- cpg_calls %>%
    dplyr::mutate(CPSR_CLASSIFICATION_CODE =
                    stringr::str_replace_all(stringr::str_replace_all(CPSR_CLASSIFICATION_CODE, "(\\|{2,})", "|"), "(^\\|)|(\\|$)", "")) %>%
    dplyr::mutate(CPSR_CLASSIFICATION_DOC =
                    stringr::str_replace_all(stringr::str_replace_all(CPSR_CLASSIFICATION_DOC, "(<br>){2,}", "<br>"), "(^(<br>))|((<br>)$)", "")) %>%
    dplyr::mutate(cpsr_score_pathogenic =
                    dplyr::if_else(stringr::str_detect(CPSR_CLASSIFICATION_CODE, "ACMG_PVS") &
    ...
    dplyr::mutate(CPSR_CLASSIFICATION =
                    dplyr::if_else(cpsr_score_pathogenic >= 3.5 & cpsr_score_pathogenic < 5,
                                   "Likely_Pathogenic", CPSR_CLASSIFICATION)) %>%
    dplyr::mutate(CPSR_CLASSIFICATION =
                    dplyr::if_else(cpsr_score_pathogenic >= 5, "Pathogenic", CPSR_CLASSIFICATION)) %>%
    dplyr::mutate(CPSR_CLASSIFICATION =
                    dplyr::if_else(cpsr_score_benign <= -2 & cpsr_score_pathogenic <= 0.5,
                                   "Likely_Benign", CPSR_CLASSIFICATION)) %>%
    dplyr::mutate(CPSR_CLASSIFICATION =
                    dplyr::if_else(cpsr_score_benign <= -5, "Benign", CPSR_CLASSIFICATION)) %>%
    dplyr::mutate(CPSR_CLASSIFICATION_SCORE =
                    dplyr::if_else(cpsr_score_benign == 0, cpsr_score_pathogenic, cpsr_score_benign)) %>%
    dplyr::mutate(CPSR_CLASSIFICATION_SCORE =
                    dplyr::if_else(cpsr_score_benign < 0 & cpsr_score_pathogenic > 0,
                                   cpsr_score_benign + cpsr_score_pathogenic, CPSR_CLASSIFICATION_SCORE)) %>%
    dplyr::mutate(CPSR_CLASSIFICATION =
                    dplyr::case_when(CPSR_CLASSIFICATION_SCORE <= -1.5 & CPSR_CLASSIFICATION_SCORE >= -4 ~ "Likely_Benign",
                                     CPSR_CLASSIFICATION_SCORE < -4 ~ "Benign",
                                     CPSR_CLASSIFICATION_SCORE <= 2.5 & CPSR_CLASSIFICATION_SCORE > -1.5 ~ "VUS",
                                     CPSR_CLASSIFICATION_SCORE >= 4.5 ~ "Pathogenic",
                                     CPSR_CLASSIFICATION_SCORE > 2.5 & CPSR_CLASSIFICATION_SCORE < 4.5 ~ "Likely_Pathogenic",
...
#' Function that assign variants to different tiers for prioritization of germline variants
#'
...
assign_cpsr_tier <- function(cpg_calls, cpsr_config, display_tags) {
  ...
  predispose_tsv_tags <- c("GENOMIC_CHANGE", "VAR_ID", "GENOTYPE",
                           ...
                           "REFSEQ_MRNA", "ONCOGENE", "TUMOR_SUPPRESSOR", "MOD",
                           "CANCER_PREDISPOSITION_MOI", "CONSEQUENCE",
                           "VEP_ALL_CSQ", "CIVIC_ID", "CIVIC_ID_SEGMENT", "PROTEIN_CHANGE",
                           "PROTEIN_DOMAIN", "HGVSp", "HGVSc", "LAST_EXON", "EXON_POSITION",
                           ...
                           "N_INSILICO_TOLERATED", "N_INSILICO_SPLICING_NEUTRAL", "N_INSILICO_SPLICING_AFFECTED",
  ...
  predispose_tsv_tags <- c(predispose_tsv_tags, evidence_codes$cpsr_evidence_code,
                           c("CPSR_CLASSIFICATION", "CPSR_CLASSIFICATION_SCORE",
  ...
  snv_indel_report <- pcgrr::init_report(config = cpsr_config, class = "snv_indel", type = "predisposition")
  ...
  for (c in c("class1", "class2", "class3", "class4", "class5")) {
    all_clinvar_calls <- all_clinvar_calls %>%
      dplyr::bind_rows(dplyr::select(snv_indel_report[["variant_set"]][[c]], VAR_ID))
  }
  ...
  cpg_calls <- cpg_calls %>%
    dplyr::filter(is.na(GLOBAL_AF_GNOMAD) | GLOBAL_AF_GNOMAD < cpsr_config[["maf_upper_threshold"]])
  n_maf_filtered <- n_nonclinvar - nrow(cpg_calls)
  rlogging::message(paste0("Ignoring n = ", n_maf_filtered,
  ...
  # rlogging::message(paste0("Ignoring n = ", n_noncoding_filtered,
  #                          " unclassified variants with a noncoding variant consequence"))
  ...
  for (c in c("class1", "class2", "class3", "class4", "class5")) {
    snv_indel_report[["variant_set"]][[c]] <-
      dplyr::bind_rows(non_clinvar_calls[[c]], snv_indel_report[["variant_set"]][[c]])

    ## If not 'classify_all' is turned on, remove CPSR classifications for existing ClinVar classifications
    if (cpsr_config[["classify_all"]] == F) {
      snv_indel_report[["variant_set"]][[c]] <- snv_indel_report[["variant_set"]][[c]] %>%
        dplyr::mutate(CPSR_CLASSIFICATION = dplyr::if_else(!is.na(CLINVAR_CLASSIFICATION),
                                                           "", as.character(CPSR_CLASSIFICATION))) %>%
        dplyr::mutate(CPSR_CLASSIFICATION_DOC = dplyr::if_else(!is.na(CLINVAR_CLASSIFICATION),
                                                               "", as.character(CPSR_CLASSIFICATION_DOC))) %>%
        dplyr::mutate(CPSR_CLASSIFICATION_CODE = dplyr::if_else(!is.na(CLINVAR_CLASSIFICATION),
                                                                "", as.character(CPSR_CLASSIFICATION_CODE))) %>%
        dplyr::mutate(CPSR_CLASSIFICATION_SCORE = dplyr::if_else(!is.na(CLINVAR_CLASSIFICATION),
                                                                 as.numeric(NA), as.numeric(CPSR_CLASSIFICATION_SCORE)))
    }
    snv_indel_report[["variant_set"]][[c]] <- snv_indel_report[["variant_set"]][[c]] %>%
      dplyr::arrange(SOURCE, desc(CANCER_PHENOTYPE), desc(CPSR_CLASSIFICATION_SCORE))
    #dplyr::select(-CANCER_PHENOTYPE)

    snv_indel_report[["variant_display"]][[c]] <-
      dplyr::select(snv_indel_report[["variant_set"]][[c]], dplyr::one_of(display_tags))
    snv_indel_report[["variant_set"]][[c]] <-
      dplyr::select(snv_indel_report[["variant_set"]][[c]], dplyr::one_of(predispose_tags))

    snv_indel_report[["variant_set"]][[c]]$DBSNP <-
      unlist(lapply(stringr::str_match_all(snv_indel_report[["variant_set"]][[c]]$DBSNP, ">rs[0-9]{1,}<"), paste, collapse = ","))
    snv_indel_report[["variant_set"]][[c]]$DBSNP <-
      stringr::str_replace_all(snv_indel_report[["variant_set"]][[c]]$DBSNP, ">|<", "")
    snv_indel_report[["variant_set"]][[c]]$GENE_NAME <-
      unlist(lapply(stringr::str_match_all(snv_indel_report[["variant_set"]][[c]]$GENE_NAME, ">.+<"), paste, collapse = ","))
    snv_indel_report[["variant_set"]][[c]]$GENE_NAME <-
      stringr::str_replace_all(snv_indel_report[["variant_set"]][[c]]$GENE_NAME, ">|<", "")
    snv_indel_report[["variant_set"]][[c]]$PROTEIN_DOMAIN <-
      unlist(lapply(stringr::str_match_all(snv_indel_report[["variant_set"]][[c]]$PROTEIN_DOMAIN, ">.+<"), paste, collapse = ","))
    snv_indel_report[["variant_set"]][[c]]$PROTEIN_DOMAIN <-
      stringr::str_replace_all(snv_indel_report[["variant_set"]][[c]]$PROTEIN_DOMAIN, ">|<", "")
    snv_indel_report[["variant_set"]][[c]]$CPSR_CLASSIFICATION_DOC <-
      stringr::str_replace_all(snv_indel_report[["variant_set"]][[c]]$CPSR_CLASSIFICATION_DOC, "<br>-", ",")
    snv_indel_report[["variant_set"]][[c]]$CPSR_CLASSIFICATION_DOC <-
      stringr::str_replace_all(snv_indel_report[["variant_set"]][[c]]$CPSR_CLASSIFICATION_DOC, "^, ", "")

    snv_indel_report[["variant_set"]][[c]] <- snv_indel_report[["variant_set"]][[c]] %>%
      dplyr::select(c("GENOMIC_CHANGE", "VAR_ID", "GENOTYPE", "SOURCE", "GENOME_VERSION",
                      ...
                      "PROTEIN_CHANGE", "PROTEIN_DOMAIN", "DBSNP", "HGVSp", "HGVSc", "LAST_EXON",
                      "EXON_POSITION","INTRON_POSITION","CDS_CHANGE", "MUTATION_HOTSPOT", "RMSK_HIT",
                      "PROTEIN_FEATURE", "EFFECT_PREDICTIONS", "LOSS_OF_FUNCTION", "DBSNP","CANCER_PHENOTYPE",
                      "CLINVAR_CLASSIFICATION", "CLINVAR_MSID", "CLINVAR_VARIANT_ORIGIN",
                      "CLINVAR_CONFLICTED", "CLINVAR_PHENOTYPE", "CLINVAR_REVIEW_STATUS_STARS"),
                    dplyr::everything())
...
#' Function that retrieves variants in genes recommended for incidental (incidental) findings
#'
...
detect_cancer_traits_clinvar <- function(cpg_calls, oncotree, umls_map) {
  if (nrow(cpg_calls) > 0 & "CLINVAR_UMLS_CUI" %in% colnames(cpg_calls) & "VAR_ID" %in% colnames(cpg_calls)) {
    ...
    n_clinvar <- cpg_calls %>% dplyr::filter(!is.na(CLINVAR_UMLS_CUI)) %>% nrow()
    ...
    if(n_clinvar > 0){
      cpg_calls_traits <- as.data.frame(
        tidyr::separate_rows(cpg_calls, CLINVAR_UMLS_CUI, sep = ",") %>%
          ...
          dplyr::mutate(CANCER_PHENOTYPE = dplyr::if_else(is.na(CANCER_PHENOTYPE),
                                                          as.integer(0),
...
summary_donut_chart <- function(variants_tsv, plot_type = "ClinVar") {
  ...
  if (nrow(variants_tsv) > 0) {
    ...
    set_clinvar <- variants_tsv %>%
      dplyr::filter(!is.na(CLINVAR_CLASSIFICATION) & !(CLINVAR_CLASSIFICATION == "NA"))
    set_other <- variants_tsv %>%
      dplyr::filter(nchar(CPSR_CLASSIFICATION) > 0 & (is.na(CLINVAR_CLASSIFICATION) | CLINVAR_CLASSIFICATION == "NA"))
    ...
    if ((plot_type == "ClinVar" & nrow(set_clinvar) > 0) | (plot_type != "ClinVar" & nrow(set_other) > 0)) {
      ...
      if (plot_type == "ClinVar") {
        if (nrow(set_clinvar) > 0) {
          ...
          m <- as.data.frame(set_clinvar %>%
            ...
            dplyr::mutate(level = factor(level, levels = pcgrr::color_palette[['pathogenicity']][['levels']])) %>%
            dplyr::arrange(level) %>%
      ...
      p <- ggplot2::ggplot(m, ggplot2::aes(x = 2, y = prop, fill = level)) +
        ...
        ggplot2::scale_fill_manual(values = pcgrr::color_palette[['pathogenicity']][['values']],
                                   labels = pcgrr::color_palette[['pathogenicity']][['levels']], drop = F) +
        ggplot2::theme_void() +
...
virtual_panel_display_html <- function(gene_df){
  ...
  while(i <= nrow(gene_df)){
    ...
    if(!is.na(panel_id)){
      gene_url <- paste0("https://panelapp.genomicsengland.co.uk/panels/",panel_id,"/",symbol)
    }
...
get_germline_biomarkers <- function(snv_indel_report, pcgr_data) {
  ...
  germline_biomarkers <- pcgr_data[["biomarkers"]][["civic"]] %>%
    dplyr::filter((ALTERATION_TYPE == "MUT" | ALTERATION_TYPE == "MUT_LOF") &
                    (stringr::str_detect(tolower(VARIANT_ORIGIN), "germline"))) %>%
    dplyr::filter(BIOMARKER_MAPPING == "exact" | BIOMARKER_MAPPING == "codon" | BIOMARKER_MAPPING == "gene") %>%
    dplyr::select(SYMBOL, ALTERATION_TYPE, CLINICAL_SIGNIFICANCE, EVIDENCE_ID,
                  EVIDENCE_DIRECTION, CITATION, RATING,
                  THERAPEUTIC_CONTEXT, CANCER_TYPE, DESCRIPTION,
                  EVIDENCE_LEVEL, EVIDENCE_TYPE, BIOMARKER_MAPPING,
  ...
  for (mapping in bmarker_mapping_levels) {
    bmarkers <- germline_biomarkers %>% dplyr::filter(BIOMARKER_MAPPING == mapping)

    if (mapping == "exact") {
      sample_calls_civic <- snv_indel_report[["variant_set"]][["tsv"]] %>%
        dplyr::filter(!is.na(CIVIC_ID))
      if (nrow(sample_calls_civic) > 0) {
        eitems <- as.data.frame(
          dplyr::select(sample_calls_civic, CIVIC_ID, VAR_ID, SYMBOL) %>%
            dplyr::rename(EVIDENCE_ID = CIVIC_ID) %>%
            tidyr::separate_rows(EVIDENCE_ID, sep = ",") %>%
            dplyr::inner_join(bmarkers, by = c("EVIDENCE_ID","SYMBOL")) %>%
            dplyr::distinct() %>%
      ...
      sample_calls_civic <- snv_indel_report[["variant_set"]][["tsv"]] %>%
        dplyr::filter(!is.na(CIVIC_ID_SEGMENT)) %>%
      ...
      if (nrow(sample_calls_civic) > 0) {
        eitems <- as.data.frame(
          dplyr::select(sample_calls_civic, CIVIC_ID_SEGMENT, VAR_ID, SYMBOL, LOSS_OF_FUNCTION) %>%
  ...
  if (nrow(clinical_evidence_items) > 0) {
    ...
    supporting_variants <- as.data.frame(
      clinical_evidence_items %>%
        ...
        dplyr::left_join(dplyr::select(snv_indel_report[["variant_set"]][["tsv"]],
                                       VAR_ID, CDS_CHANGE, GENE_NAME, GENOMIC_CHANGE, SYMBOL),
                         by = c("VAR_ID")) %>%
    ...
    rlogging::message(paste0(nrow(clinical_evidence_items),
                             " clinical evidence item(s) found .. mapping = ", mapping))
    ...
    for (type in c("prognostic", "diagnostic", "predictive", "predisposing")) {
      clin_eitems_list[[type]] <- clinical_evidence_items %>%
        ...
        dplyr::select(SYMBOL, GENE_NAME, CANCER_TYPE, CLINICAL_SIGNIFICANCE,
                      EVIDENCE_LEVEL, RATING, dplyr::everything()) %>%
        pcgrr::remove_cols_from_df(cnames = c("ALTERATION_TYPE",
                                              "EVIDENCE_ID", "EVIDENCE_TYPE"))
    }
  }

  snv_indel_report$clin_eitem <- clin_eitems_list
  return(snv_indel_report)
