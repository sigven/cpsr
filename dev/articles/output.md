...
| `SPLICE_EFFECT` | Effect of splicing, from MutSpliceDB and/or MaxEntScan, combined into a single comma-separated string. MutSpliceDB component format: `<ENTREZGENE>|<SYMBOL>|<REFSEQ_TRANSCRIPT_ID>|<HGVSc>|<EFFECT>|<AFFECTED_EXONS>|<SOURCE>`. MaxEntScan component format: `MaxEntScan|<stratum>|<evidence_tier>` (see `MAXENTSCAN` below for details) |
| `EFFECT_PREDICTIONS` | Insilico predictions variant effect on protein function and pre-mRNA splicing from [database of non-synonymous functional predictions - dbNSFP](https://www.dbnsfp.org/). Predicted effects are provided by different sources/algorithms (separated by `&`), `T` = Tolerated, `N` = Neutral, `D` = Damaging |
...
| 48\. `MAXENTSCAN` | MaxEntScan splice site impact summary for canonical (±1/±2 bp) and non-canonical (donor +3 to +6, acceptor -3 to -20) splice positions. Format: `MaxEntScan|<stratum>|<evidence_tier>`. `<stratum>` identifies the splice-site position category (e.g. `Donor_+5`, `Acceptor_PPT_Close`, `Canonical_Site`). `<evidence_tier>` reflects predicted impact strength, from strongest to weakest: `Strong` \> `Moderate` \> `Supporting` \> `No_Call` \> `Uninformative` (`.` for canonical sites, which are always high-confidence). Only variants with a `Strong` tier at non-canonical positions are additionally flagged as loss-of-function |
